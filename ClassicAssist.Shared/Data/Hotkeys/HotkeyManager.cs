    ...
    public class HotkeyManager : INotifyPropertyChanged
    {
        ...
        public bool OnHotkeyPressed( Key keys )
        {
            lock ( _lock )
            {
                ...
                bool down = Convert.ToBoolean(GetKeyState(KeysToVirtualKey(modifier)) & 0x8000);
        ...
        private VirtualKeyStates KeysToVirtualKey(Key modifier )
        {
            //TODO Enter keys / find better way
            switch ( modifier )
            {
                case Key.LeftAlt:
                    return VirtualKeyStates.VK_LMENU;
                case Key.Right:
                    return VirtualKeyStates.VK_RMENU;
                case Key.LeftCtrl:
                    return VirtualKeyStates.VK_CONTROL;
                case Key.RightCtrl:
                    return VirtualKeyStates.VK_CONTROL;
                case Key.LeftShift:
                    return VirtualKeyStates.VK_LSHIFT;
                case Key.RightShift:
                    return VirtualKeyStates.VK_RSHIFT;
            }
